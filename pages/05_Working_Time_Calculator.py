...
from utils.constants import (
    CI_TO_MBQ, GAMMA_DOSE_CONSTANT, NEUTRON_SOURCES,
...
with col_in:
    ...
    if dr_mode == "Calculate from activity":
        ...
        with a_col:
            act_val = st.number_input("Activity", min_value=0.0, value=1.0, step=0.1, key="wt_act")
        with u_col:
            act_unit = st.selectbox("Unit", UNITS, index=UNITS.index('MBq'), key="wt_unit")

        dist_m = st.number_input(
            "Distance from source (m)", min_value=0.1, value=1.0, step=0.1, key="wt_dist",
        )
    ...
    limit_sel = st.selectbox("Dose limit scheme", list(ANNUAL_LIMITS_MSV.keys()), key="wt_limit")
    if limit_sel == "Custom":
        dose_limit_mSv_y = st.number_input(
            "Annual dose limit (mSv/y)", min_value=0.1, value=20.0, step=1.0, key="wt_custom",
        )
    else:
    ...
    occupancy = st.slider(
        "Occupancy factor", min_value=0.05, max_value=1.0, value=1.0, step=0.05, key="wt_occ",
    )
...
with col_out:
    ...
    if dr_working <= 0:
        ...
        annual_budget_uSv = dose_limit_mSv_y * 1000.0          # μSv/y
        ...
        if t_max_day_min < 1:
            st.warning(
                f"⚠️ **Extreme dose rate:** maximum permissible time is less than 1 minute/day "
                f"({t_max_day_min * 60:.1f} s/day). "
                "Ensure adequate shielding and maximise working distance before approaching this source."
            )
        ...
        if dr_mode == "Calculate from activity":
            ...
            for d in distances:
                if source_type == "Gamma":
                    ...
                    gr_d = (gamma_dr_from_activity(act_MBq, ns['gamma_dose_constant'], d)
                            if ns['gamma_dose_constant'] > 0 else 0.0)
                    dr_d = nr_d + gr_d
        ...
        src_label = (
            f"{isotope_sel} gamma"
            if dr_mode == "Calculate from activity" and source_type == "Gamma"
