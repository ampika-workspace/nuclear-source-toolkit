...
with right:
    if not isotope:
        ...
        if iaea is None:
            ...
            if days_query < 0:
                st.info(
                    f"ℹ️ **Back-calculation:** {query_date} is before the reference date "
                    f"({ref_date}). The result shows the activity the source held at that "
                    "earlier date — this is physically valid."
                )
