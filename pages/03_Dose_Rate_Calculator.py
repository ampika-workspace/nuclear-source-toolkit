...
# ── Physics helpers ───────────────────────────────────────────────────────────
def gamma_dose_rate(activity_MBq, gamma_const, distance_m):
    return gamma_const * activity_MBq / (distance_m ** 2)

def neutron_dose_rate(activity_MBq, yield_per_MBq, h_phi, distance_m):
    S    = activity_MBq * yield_per_MBq
    r_cm = distance_m * 100
    return S * h_phi * 3600 / (4 * np.pi * r_cm ** 2 * 1e6)

def safe_distance(dr_at_1m, limit_uSvh):
...
def show_nuclear_properties(isotope_label: str, context_label: str = None):
    """Render the full nuclear properties expander for a given isotope."""
        ...
        if props['daughter']:
            ...
            if daughter_basic:
                ...
                if 'stable' in d_stable or float(daughter_basic.get('half_life_sec', 0) or 0) > 3e15:
                    ...
                    st.warning(
                        f"Daughter **{props['daughter']}** is radioactive (t½ = {daughter_basic['half_life_hum']}). "
    ...
    with tab_bm:
        ...
        df_beta = pd.concat([d for d in [df_bm, df_bp] if d is not None and not d.empty], ignore_index=True) if any(d is not None for d in [df_bm, df_bp]) else None
...
# ── Source selection (full width so both panels can use the variables) ────────
st.markdown("### Source type")
source_type = st.radio("Source type", ["Gamma", "Neutron"], horizontal=True, label_visibility="collapsed")
st.divider()

if source_type == "Gamma":
    st.markdown("### Isotope")
    isotope_sel = st.selectbox("Isotope", list(GAMMA_DOSE_CONSTANT.keys()) + ["Other (enter Γ manually)"], label_visibility="collapsed")
    if isotope_sel == "Other (enter Γ manually)":
        gamma_const   = st.number_input("Dose rate constant Γ (μSv·m²·h⁻¹·MBq⁻¹)",
    ...
    source_sel    = st.selectbox("Neutron source type", list(NEUTRON_SOURCES.keys()), label_visibility="collapsed")
    ns            = NEUTRON_SOURCES[source_sel]
    isotope_label = source_sel
    props_isotope = ns['parent_isotope']
    _bd = _basic(ns['parent_isotope'])
    if _bd:
        st.caption(f"Matrix isotope: **{ns['parent_isotope']}** · Half-life: **{_bd['half_life_hum']}** · Decay mode: **{_bd['decay_mode']}**")

st.divider()

# ── Nuclear properties (full width, always visible once source is chosen) ─────
if props_isotope:
    context = f"{props_isotope} (matrix isotope of {source_sel})" if source_type == "Neutron" else props_isotope
...
with right:
    ...
    if source_type == "Gamma":
        dr       = gamma_dose_rate(activity_MBq, gamma_const, distance_m)
        dr_at_1m = gamma_dose_rate(activity_MBq, gamma_const, 1.0)
        m1, m2, m3 = st.columns(3)
        m1.metric(f"Dose rate at {distance_m} m", f"{dr:.4f} μSv/h")
        m2.metric("", f"{dr / 1000:.6f} mSv/h")
        m3.metric("Annual estimate (2000 h)", f"{dr * 2000 / 1000:.3f} mSv/yr")
    else:
        nr = neutron_dose_rate(activity_MBq, ns['neutron_yield_per_MBq'], ns['h_phi_pSv_cm2'], distance_m)
        gr = gamma_dose_rate(activity_MBq, ns['gamma_dose_constant'], distance_m) if ns['gamma_dose_constant'] > 0 else 0.0
        dr = nr + gr
        nr_at_1m = neutron_dose_rate(activity_MBq, ns['neutron_yield_per_MBq'], ns['h_phi_pSv_cm2'], 1.0)
        gr_at_1m = gamma_dose_rate(activity_MBq, ns['gamma_dose_constant'], 1.0) if ns['gamma_dose_constant'] > 0 else 0.0
        ...
        m2.metric(f"Gamma ({ns['parent_isotope']} intrinsic)", f"{gr:.4f} μSv/h",
                  help=f"From {ns['parent_isotope']} gamma emission — part of the source matrix, not neutron interaction")
    ...
    fig   = go.Figure()
    if source_type == "Gamma":
        fig.add_trace(go.Scatter(x=d_arr,
                                  y=[gamma_dose_rate(activity_MBq, gamma_const, d) for d in d_arr],
                                  mode='lines', name=f'{isotope_label} (γ)',
                                  line=dict(color='steelblue', width=2)))
    else:
        nr_arr = [neutron_dose_rate(activity_MBq, ns['neutron_yield_per_MBq'], ns['h_phi_pSv_cm2'], d) for d in d_arr]
        gr_arr = [gamma_dose_rate(activity_MBq, ns['gamma_dose_constant'], d) if ns['gamma_dose_constant'] > 0 else 0.0 for d in d_arr]
        fig.add_trace(go.Scatter(x=d_arr, y=[n+g for n,g in zip(nr_arr,gr_arr)],
                                  mode='lines', name='Total', line=dict(color='steelblue', width=2)))
    ...
    fig.add_scatter(x=[distance_m], y=[dr], mode='markers+text',
                    marker=dict(size=10, color='gold', symbol='diamond'),
                    text=[f'{dr:.3f} μSv/h'], textposition='top right',
                    name=f'Selected: {distance_m} m')
    fig.update_layout(title=f"Dose Rate vs Distance — {isotope_label} ({activity_val} {input_unit})",
    ...
    for d in [0.1, 0.25, 0.5, 0.75, 1.0, 1.5, 2.0, 3.0, 5.0, 7.0, 10.0]:
        if source_type == "Gamma":
            dr_d = gamma_dose_rate(activity_MBq, gamma_const, d)
            rows.append({"Distance (m)": d, "Dose rate (μSv/h)": round(dr_d, 4),
                         "Dose rate (mSv/h)": round(dr_d/1000, 7),
                         "Annual estimate (mSv/yr)": round(dr_d*2000/1000, 4)})
        else:
            nr_d = neutron_dose_rate(activity_MBq, ns['neutron_yield_per_MBq'], ns['h_phi_pSv_cm2'], d)
            gr_d = gamma_dose_rate(activity_MBq, ns['gamma_dose_constant'], d) if ns['gamma_dose_constant'] > 0 else 0.0
            rows.append({"Distance (m)": d,
    ...
    st.download_button("⬇️ Download table (CSV)", df_table.to_csv(index=False),
